...
class DataHandler:
    ...
    def LoadData(self):
        ...
        with torch.no_grad():
            emb_u, _ = temp_model(ui_graph, dgl.from_scipy(self.dataset['trust']))  # uu_graph temporary
            emb_u = emb_u[:self.n_user]  # Keep only users
        ...
        print("Number of nodes in uu_graph:", self.uu_graph.num_nodes())
        print("Number of edges in uu_graph:", self.uu_graph.num_edges())

        # Compute density (optional, useful for diagnostics)
        n = self.uu_graph.num_nodes()
        e = self.uu_graph.num_edges()
        density = e / (n * (n - 1))
        print(f"UU graph density: {density:.6f}")
